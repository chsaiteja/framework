...
@Component({
	selector: 'cci-quick-intro',
	templateUrl: './cci-quick-intro.layout.html',
})

export class CCIQuickIntroComponent implements OnInit, OnDestroy {
	cciQuesNames; /*Declare for storing questions variable*/
	accountId; /* Declare for Store accountID value */
	cciQuickQuesResponses; /*Declare for storing responses*/
	...
	/**
	 * This method is for getting the responses for CCIQuick from API
	 */
	getCCIQuickQuesRes() {
		try {
			...
			this.dispatchStore.dispatch({
				type: "GET_QUESTION_RESPONSES", payload: {
					...
					body_Params: {}, endUrlVal: 'CCIQuick', setVal: 'questions', text: ''
				}
			});
		...
		try {
			...
			const data = {
				input_data: [
					{
						'param_type': 'path',
						'params': ['start', this.accountId, 'CCIQuick']
					},
					{
						'param_type': 'query',
						'params': {}
	...
	/**
	 * This method is to navigate to CCIQuick assessement page
	*/
	startAssessment() {
