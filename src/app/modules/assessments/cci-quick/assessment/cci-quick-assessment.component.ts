...
@Component({
	selector: 'cci-quick-assessment',
	templateUrl: './cci-quick-assessment.layout.html',
})

export class CCIQuickAssessmentComponent implements OnInit, OnDestroy {
	...
	showNxtCCIQuick = false; /**showNxt boolean variable used for showing the next button */
	...
	cciQuickQuesResponses; /**Declare for  Storing the responses getting using session Storage */
	...
	*/
		...
		try {
			...
			this.dispatchStore.dispatch({
				type: "GET_QUESTION_TEXT", payload: {
					...
					body_Params: {}, endUrlVal: 'CCIQuick'
				}
			});
	...
	/**
	 * This function is for starting the CCIQuick assessement.
	*/
	startEntrePreExam() {
	...
	*/
	nextCCIArrow(cnt) {
		...
		if ((this.cnt + 1) == this.staticVal) {
			this.showNxtCCIQuick = false;
		} else {
			this.showNxtCCIQuick = true;
		}
		...
		try {
			if (this.cnt <= this.cciQuesNames.commonIntroText.questions.length) {
				...
				if (this.cnt == this.cciQuesNames.commonIntroText.questions.length) {
					...
					this.remaining = 0;
					this.utils.showLoading();
					this.storageService.sessionStorageSet('answerSetCCIQuick', this.answerSet.toString());
					this.trackEvnt.getCCIQuickResult(this.answerSet.toString(), this.storageService.sessionStorageGet('save_Par_UserNotes'));
				}
				if (this.cnt == this.staticVal) {
