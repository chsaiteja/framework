...
@Component({
	selector: 'cci-quick-restore',
	templateUrl: './cci-quick-restore.layout.html',
})
export class CCIQuickRestoreComponent implements OnInit, OnDestroy {
	...
	ngOnInit() {
		...
		/**Call the getCCIQuickAnswerSet method for getting the answetSet value at the time of loading */
		this.getCCIQuickAnswerSet();
	}
	 ...
	 */
	restoreQuestionsCCIQuick(answerSet, usernotes) {
		try {
			...
			this.serverApi.callApi([this.apiJson]).subscribe((response) => {
				...
				if (this.restoreQuesArr.length == 0) {
					this.storageService.sessionStorageSet('answerSetCCIQuick', response[0].Result.answers);
					 ...
					 */
					this.assssementSer.getCCIQuickResult(response[0].Result.answers, response[0].Result.userNotes);
				} else {
