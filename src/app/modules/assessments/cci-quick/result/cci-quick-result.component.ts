...
@Component({
	selector: 'cci-quick-result',
	templateUrl: './cci-quick-result.layout.html',
})
export class CCIQuickResultComponent implements OnInit, OnDestroy {

	introVal; /**Declare for storing the assessmentintro text from store.*/
	cciQuickresultArr = []; /*Declare for storing result*/
	logId; /**Declare for Storing the value for logID */
	ccikeys = []; /**Declare for Storing the value of cluster Id's */
	cciQuickResult = []; /**Declare for pushing the result */
	...
	ngOnInit() {
		...
		this.cciQuickresultArr = JSON.parse(this.storageService.sessionStorageGet('CCIQuickResult'));
		for (let k = 0; k < ref.cciQuickresultArr.length; k++) {
			let val = this.trackEvnt.clustDetails(ref.cciQuickresultArr[k].clusterID);
			/**Hear we are comparing the Cluster ID's */
			ref.cciQuickresultArr[k].clusterIconValue = (val.clusterIconValue);
			ref.cciQuickresultArr[k].clusterBgColor = (val.clusterBgColor);
			ref.cciQuickresultArr[k].clusterName = (val.clusterName);
		}
		this.getIntroText();
		this.CCIjsonSort(JSON.parse(this.storageService.sessionStorageGet('CCIQuickResult')));
	}
	...
	getIntroText() {
		...
		this.dispatchStore.dispatch({
			type: "GET_INTRO_TEXT", payload: {
				...
				body_Params: {}, endUrlVal: 'pageText/cciQuick', setVal: 'commonIntro', text: ''
			}
		});
	...
	CCIjsonSort(res) {
		try {
			...
			for (let i = 0; i < cciScore.length; i++) {
				...
				for (let k = 0; k < this.cciQuickresultArr.length; k++) {
					if (this.cciQuickresultArr[k].clusterID == this.ccikeys[i]) {
						this.cciQuickResult.push({
							cciQuickClusterId: this.ccikeys[i],
							cciQuickScore: cciScore[i],
							cciQuickIcon: this.cciQuickresultArr[k].clusterIconValue,
							cciQuickBgColor: this.cciQuickresultArr[k].clusterBgColor,
							cciQuickClusterName: this.cciQuickresultArr[k].clusterName
						});
					}
				}
			}
